...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      // Sanity-check restored values: if pan is way off-screen or zoom is
      // pathological (saved while the canvas had been panned into the void),
      // ignore it and start at origin so the user can actually see the content.
      const inBounds = s
        && Number.isFinite(s.x) && Math.abs(s.x) < 10000
        && Number.isFinite(s.y) && Math.abs(s.y) < 10000
        && Number.isFinite(s.scale) && s.scale > 0.2;
      if (inBounds) {
        tf.current = { x: s.x, y: s.y, scale: Math.min(maxScale, Math.max(minScale, s.scale)) };
        apply();
      } else if (s) {
        // Clear the bad state so it doesn't keep resurrecting itself.
